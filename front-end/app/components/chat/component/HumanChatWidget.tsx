...
import { MessageCircle, X, Plus, Send, MoreVertical, AlignJustify } from "lucide-react";
...
export default function HumanChatWidget() {
  const PAGE_SIZE = 15;
  ...
  // Sự kiện mở chat từ bên ngoài (chỉ human)
  type OpenChatDetail = { receiverId: number; receiverName?: string; avatar?: string | null };
  useEffect(() => {
    const onOpenChat = (ev: Event) => {
      const e = ev as CustomEvent<OpenChatDetail>;
      const rid = Number(e.detail?.receiverId);
      if (!rid || Number.isNaN(rid) || rid <= 0) return;

      setShowList(true);
      setActiveChat(true);
      setReceiver((prev) => {
        if (prev?.id === rid) return prev;
        return { id: rid, name: e.detail?.receiverName || "Người dùng", avatar: e.detail?.avatar ?? null } as User;
      });
      setIsInitialLoad(true);
      setLastReceiver(rid);
    };
    window.addEventListener("open-chat-box", onOpenChat as EventListener);
    return () => window.removeEventListener("open-chat-box", onOpenChat as EventListener);
  }, []);

  // Mở lần đầu → chọn cuộc trước (server/local) hoặc contact đầu
  useEffect(() => {
    if (!showList || receiver) return;
    const tk = token || localStorage.getItem("token") || Cookies.get("authToken");
    if (!tk || !currentUser) return;

    (async () => {
      let targetId: number | null =
        (typeof currentUser.last_conversation_id === "number" ? currentUser.last_conversation_id : null) ?? null;
      ...
      if (targetId == null) {
        try {
          ...
          const humans: User[] = (res.data || []).filter((u: User) => Number(u.id) > 0);
          if (humans.length > 0) targetId = humans[0].id;
        } catch {}
      }
  ...
  // Đổi receiver → hủy request đang chạy
  useEffect(() => {
    fetchAbortRef.current?.abort();
    isFetchingRef.current = false;
    setLoading(false);
    ...
    <>
      ...
      <div className="fixed right-5 bottom-5 z-[9999] md:right-5 md:bottom-5">
        <button
          ...
          className={`${showList ? "hidden" : "block"} relative w-14 h-14 rounded-full bg-gradient-to-br from-[#e14b4b] to-[#c93434] text-white shadow-2xl ring-2 ring-white/50 hover:ring-white transition-all duration-200 hover:scale-105 active:scale-95 flex items-center justify-center`}
        ...
        >
          ...
          {unreadCount > 0 && !showList && (
            <>
              ...
              <span className="absolute -top-1.5 -right-1.5 inline-flex h-5 w-5 rounded-full bg-red-500/60" />
              <span className="absolute -top-2 -right-2 inline-flex h-7 w-7 rounded-full bg-red-500/40" />
            </>
          )}
        </button>
      </div>
      ...
      {/* Chat Window */}
      {showList && (
        <div className="fixed z-[9998] inset-x-2 bottom-2 md:inset-auto md:bottom-5 md:right-24 w-auto md:w-[760px] max-w-[95vw] h-[75vh] md:h-[500px] bg-white/95 backdrop-blur rounded-2xl border border-gray-200/70 shadow-[0_10px_40px_-10px_rgba(219,68,68,0.35)] overflow-hidden">
            ...
            <div className={`w-full md:w-[290px] border-b md:border-b-0 md:border-r bg-gray-50/70 sm:flex flex-col max-h-full md:max-h-none ${isContactListOpen ? "flex" : "hidden"}`}>
              <div className="px-4 py-3 bg-gradient-to-r from-[#db4444] to-rose-500 text-white">
                <div className="flex items-center justify-between">
                  ...
                  <div className="font-semibold">Liên hệ gần đây</div>
                  <button onClick={() => setShowList(false)} className="w-9 h-9 rounded-full hover:bg-white/10 flex md:hidden items-center justify-center">
                    <X size={16} />
                  </button>
                </div>
                ...
                {filteredContacts.map((user) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (isContactListOpen) toggleContactList();
                    }}
