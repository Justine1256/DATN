...
export default function CartSummarySection({ cartItems }: Props) {
  ...
  const subtotal = useMemo(() => {
    return cartItems.reduce(
      (acc, item) => acc + item.quantity * (item.variant?.price || item.product?.price || 0),
      0
    );
  }, [cartItems]);
  ...
  const voucherDiscount = 0; // Đã bỏ phần mã giảm giá
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-black min-h-[250px]">
      {/* Chỉ còn phần tóm tắt đơn hàng */}
      <div className="border rounded-md p-5 h-full flex flex-col justify-between text-sm md:col-span-2">
        <div>
