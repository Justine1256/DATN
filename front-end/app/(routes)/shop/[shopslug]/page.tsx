...
/** Kiểu trả về từ API cho shop (tránh trùng tên với kiểu Shop của ShopCard) */
interface ShopApi {
  id: number;
  name: string;
  description: string;
  logo: string;
  phone: string;
  rating: string | number; // API có thể trả string
  total_sales: number;
  created_at: string;
...
// Kiểu prop chính xác mà ShopCard cần cho prop `shop`
type ShopCardShop = ComponentProps<typeof ShopCard>["shop"];

/** Kiểu dữ liệu đã chuẩn hoá cho UI (ép số) */
type NormalizedProduct = Omit<
  Product,
  "price" | "sale_price" | "rating_avg" | "createdAt" | "rating" | "oldPrice"
> & {
  price: number;
  sale_price?: number;
  rating_avg?: number;
...
export default function ShopPageAntd() {
  ...
  } = useSWR(slug ? getProductsUrl() : null, fetcher, {
  ...
  // ---- Derive data ----
  const shopApi: ShopApi | null = (shopData?.shop as ShopApi) || null;
  const categories: Category[] = (categoryData?.categories as Category[]) || [];

  const products: Product[] = (productData?.products?.data as Product[]) || [];
  const pagination = {
  ...
  // Map product fields -> NormalizedProduct (ép kiểu số để tránh lỗi type)
        ...
        <Content style={{ background: "transparent" }}>
          ...
          <Layout style={{ background: "transparent" }}>
            ...
            <Sider
              ...
              breakpoint="lg"
              collapsedWidth={0}
              style={{ background: "transparent", paddingRight: 16, minHeight: "100%" }}
                ...
                <Row gutter={[8, 8]} align="middle">
                  ...
                  <Col xs={24} md={4}>
                    <Space style={{ width: "100%" }}>
                      ...
                      <Button
                        className="block lg:hidden"   // mobile/tablet thấy, desktop ẩn
                        type="primary"
                        icon={<FilterOutlined />}
