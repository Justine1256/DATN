    ...
    try {
      const res = await fetch(`${API_BASE_URL}/vnpay/create`, {
        ...
        body: JSON.stringify({
          ...
          orderInfo: values.order_info || "",  // chuỗi mô tả
          // KHÔNG gửi return_url từ FE – BE ký bằng env cố định
        }),
      });
      ...
      if (res.ok && data.payment_url) {
        // Redirect nguyên URL BE trả về, KHÔNG parse/rebuild.
      ...
      <Card>
        <Space direction="vertical" size="large" style={{ width: "100%" }}>
          <div style={{ textAlign: "center" }}>
            ...
            <Alert
              message="FE chỉ gọi BE & redirect URL – không chứa secret, không tự verify"
              type="info"
              showIcon
            />
          </div>
          ...
          <Form
            ...
            initialValues={{
              amount: 100000,
              order_info: "Test payment - no FE signature",
            }}
          >
            <Form.Item
              label="Số tiền (VND)"
            ...
            <Form.Item label="Thông tin đơn hàng" name="order_info">
              <Input placeholder="Mô tả đơn hàng" />
            </Form.Item>
            ...
            <Card type="inner" title="Kết quả">
              {result.ok ? (
                <Space direction="vertical" style={{ width: "100%" }}>
                  <Alert
                    message="Tạo payment URL thành công – đang chuyển đến VNPay"
                    type="success"
                    showIcon
                  />
                ...
                <Alert
                  ...
                  description={result.error ? String(result.error) : JSON.stringify(result.data)}
                  type="error"
                  showIcon
                />
              )}
