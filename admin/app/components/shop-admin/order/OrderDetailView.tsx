...
export default function OrderDetailView({
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            {/* Lớp popup chứa nội dung cuộn */}
            <div className="w-full max-w-6xl max-h-[90vh] mx-4 relative bg-white rounded-3xl border border-gray-200">
                {/* Nội dung cuộn */}
                <div className="overflow-y-auto h-full max-h-[90vh] p-8">
                    <button
                        onClick={onClose}
                                ...
                                <tbody>
                                    {products.map((p, index) => {
                                        ...
                                        return (
                                            <tr
                                                key={p.id}
                                                className={`border-b border-gray-100 hover:bg-gray-50/50 transition-colors duration-150 ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/30'
                                                    }`}
                                            >
                        ...
                        <div className="flex justify-between items-center">
                            <div>
                                ...
                                <p className="text-sm text-gray-600">
                                    Tổng số lượng: {products.reduce((acc, p) => acc + p.quantity, 0)}
                                </p>
                            </div>
