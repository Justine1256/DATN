...
const chartData: ChartDatum[] = [
  { name: 'Th1', revenue: 45000, orders: 240, customers: 180 },
  { name: 'Th2', revenue: 38000, orders: 198, customers: 165 },
  { name: 'Th3', revenue: 52000, orders: 280, customers: 220 },
  { name: 'Th4', revenue: 48000, orders: 258, customers: 195 },
  { name: 'Th5', revenue: 61000, orders: 320, customers: 280 },
  { name: 'Th6', revenue: 55000, orders: 290, customers: 245 },
  { name: 'Th7', revenue: 68000, orders: 385, customers: 320 },
  { name: 'Th8', revenue: 72000, orders: 410, customers: 350 },
  { name: 'Th9', revenue: 65000, orders: 365, customers: 310 },
  { name: 'Th10', revenue: 78000, orders: 445, customers: 390 },
  { name: 'Th11', revenue: 82000, orders: 480, customers: 420 },
  { name: 'Th12', revenue: 95000, orders: 550, customers: 485 }
];
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-white p-4 rounded-xl shadow-lg border border-gray-100 min-w-[200px]">
        <p className="font-semibold text-gray-900 mb-2">{`Tháng: ${label}`}</p>
        {payload.map((entry, index) => (
          <div key={index} className="flex items-center justify-between mb-1">
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full" style={{ backgroundColor: entry.color }} />
              <span className="text-sm text-gray-600 capitalize">
                {entry.dataKey === 'revenue' ? 'Doanh thu' : entry.dataKey === 'orders' ? 'Đơn hàng' : 'Khách hàng'}
              </span>
            </div>
...
export default function DashboardChart() {
  ...
  const [timeRange] = useState('12 tháng');
    ...
    <div className={`bg-white rounded-2xl shadow-sm border border-gray-100 transition-all duration-300 hover:shadow-lg ${isFullscreen ? 'fixed inset-4 z-50' : 'col-span-12'
      ...
      <div className="p-6 border-b border-gray-100">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-xl font-bold text-gray-900 mb-1">Hiệu suất sản phẩm</h2>
            <p className="text-sm text-gray-600">Theo dõi chỉ số kinh doanh theo thời gian</p>
          </div>
          ...
          <div className="flex items-center gap-2">
            {(['revenue', 'orders', 'customers'] as DataKey[]).map(metric => (
              ...
              >
                ...
                {metric === 'revenue' ? 'Doanh thu' : metric === 'orders' ? 'Đơn hàng' : 'Khách hàng'}
              </button>
            ))}
          </div>
          ...
          {isLoading && (
            <div className="absolute inset-0 bg-white/80 backdrop-blur-sm rounded-xl flex items-center justify-center z-10">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm text-gray-600">Đang tải biểu đồ...</span>
              </div>
            </div>
          )}
